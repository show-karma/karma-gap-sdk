...
import { Hex, IAttestationResponse, ICommunityResponse, IGrantResponse, IMilestoneResponse, IProjectResponse } from "./types";
...
export class GapIndexerApi extends AxiosGQL {
  ...
  async searchProjects(query: string): Promise<IProjectResponse[]> {
    const { data } = await this.client.get<IProjectResponse[]>(
  ...
  async projects(): Promise<IProjectResponse[]> {
    const { data } = await this.client.get<IProjectResponse[]>(Endpoints.project.all());
  ...
  async milestonesOf(uid: Hex) {
    const response = await this.client.get(
      Endpoints.project.milestones(uid)
    );
